...
//`uuid` VARCHAR(64) NOT NULL PRIMARY KEY);'
  ...
  Future<void> init() async {
    database ??= await openDatabase(
      join(await getDatabasesPath(), 'key_data.db'),
    );
    await _createKeyLog();
    return;
  }
  ...
  Future<void> updateUserDataColumn(String uuid,
      {String? dataString, Map? data}) async {
    if (data != null) {
      dataString = jsonEncode(data);
    }
    await database?.update(_mainTableName, {'data': dataString},
        where: 'uuid = ?', whereArgs: [uuid]);
  }

  Future<Map<String, Object?>?> getUserData(String uuid) async {
    var result = await database
        ?.query(_mainTableName, where: 'uuid = ?', whereArgs: [uuid]);
  ...
  Future<void> updateUserKey(User user,
    ...
    if (privateKey != null) {
      if (EncryptionUtil.isKeyValid(KeyType.private, privateKey)) {
        user.update(
            KeyType.private, CryptoUtils.rsaPrivateKeyFromPem(privateKey));
    ...
    if (updateMap.keys.isEmpty) {
      ELog.i(
          "There was an attempted key update with blank keys! User: ${user.uuid}");
      return;
    }
    await database?.update(_mainTableName, updateMap,
