...
class HandShaker {
  ...
  Future<String?> _decrypt(WebSocket socket, List<String> messages) async {
    if (_keyPairCache.containsKey(_getUUID(socket))) {
      Encrypter encrypter = EncryptionUtil.createEncrypter(null,
          _keyPairCache[_getUUID(socket)!]!.privateKey as rsa.RSAPrivateKey);
  ...
  void _handleDecryptedMessage(WebSocket socket, String message) async {
    if (!JSONValidate.isValidJSON(message, requiredKeys: [
      MagicNumber.CODE_1_DELIM,
      MagicNumber.CODE_2_DELIM,
      MagicNumber.PUSH_NOTIFICATION_DELIM
    ])) {
    ...
    if (_handshaking.containsKey(socket) &&
        _authData.containsKey(_getUUID(socket))) {
      AuthenticationData authData = _authData[_getUUID(socket)]!;
      if (authData.codes[0] == c && authData.codes[1] == d) {
        if (authData.expiration > DateTime.now().millisecondsSinceEpoch) {
          ...
          if (user == null) {
            ELog.e(
                "User successfully authenticated, but cannot be found in database!");
          ...
          if (_keyPairCache.containsKey(uuid)) {
            ...
            await KeyHandler().updateUserKey(user,
                privateKey: CryptoUtils.encodeRSAPrivateKeyToPem(
                    _keyPairCache[uuid]!.privateKey as rsa.RSAPrivateKey));
          }
          await user.identified(socket);
          user.sendMessage(
              jsonEncode({MagicNumber.STATUS: StatusCode.SUCCESS}));
          ...
          socket.add(
              jsonEncode({MagicNumber.STATUS: StatusCode.HANDSHAKE_TIME_OUT}));
          _handshaking.remove(socket);
          _keyPairCache.remove(_getUUID(socket));
          _authData.remove(_getUUID(socket));
        }
      ...
      socket
          .add(jsonEncode({MagicNumber.STATUS: StatusCode.HANDSHAKE_TIME_OUT}));
      _handshaking.remove(socket);
      _keyPairCache.remove(_getUUID(socket));
      _authData.remove(_getUUID(socket));
    }
  ...
  void _handleHandshake(WebSocket socket, dynamic data) {
    if (!JSONValidate.isValidJSON(data,
      ...
      ELog.e(data);
      ELog.e(
          "Handshake response does not contain message compilation parts for ${_getUUID(socket)}");
      return;
    }
  ...
  void _handshakeClient(WebSocket socket, dynamic data) async {
    if (!JSONValidate.isValidJSON(data,
        requiredKeys: [MagicNumber.USER_ID_DELIM])) {
    ...
    if (JSONValidate.isValidJSON(data, requiredKeys: [
      MagicNumber.PUBLIC_KEY_DELIM,
      MagicNumber.USER_ID_DELIM
    ])) {
      // p = public_key, u = desired uuid
      User? user = await UserHandler().createUser(
          json[MagicNumber.USER_ID_DELIM], json[MagicNumber.PUBLIC_KEY_DELIM]);
      if (user == null) {
        socket.add(
            jsonEncode({MagicNumber.STATUS: StatusCode.FAILED_TO_CREATE_USER}));
        ELog.e(
            "User account could not be created. createUser(${json[MagicNumber.USER_ID_DELIM]}, ${json[MagicNumber.PUBLIC_KEY_DELIM]}) failed.");
        return;
      } else {
        asym.AsymmetricKeyPair keys = CryptoUtils.generateRSAKeyPair();
        _keyPairCache[user.uuid] = keys;
        includePublicKey = CryptoUtils.encodeRSAPublicKeyToPem(
            keys.publicKey as rsa.RSAPublicKey);
      }
    }
  ...
  void _sendAuthData(WebSocket socket, {String? publicKey}) async {
    ...
    if (user == null) {
      ELog.e(
          "Trying to send auth data to a user who doesn't exist! User: ${_handshaking[socket]}");
      socket.add(jsonEncode({MagicNumber.STATUS: StatusCode.SERVER_ERROR}));
      return;
    }
    if ((user.communicationLevel == CommunicationLevel.canSend &&
            _keyPairCache.containsKey(user.uuid)) ||
        user.communicationLevel == CommunicationLevel.full) {
      Map sending = {
        MagicNumber.CODE_1_DELIM: _authData[user.uuid]!.codes[0],
        MagicNumber.CODE_2_DELIM: _authData[user.uuid]!.codes[1]
      };
      if (publicKey != null) {
        sending[MagicNumber.PUBLIC_KEY_DELIM] = publicKey;
      }
      List<String> encryptedPieces =
          user.getEncryptedPieces(jsonEncode(sending));
      socket
